...
def index():
    ...
    genre_counts = df.groupby('genre')['message'].count().sort_values(ascending=False)
    ...
    num_category = df.drop(['id', 'message', 'original', 'genre'], axis=1).sum(axis=1).value_counts()
    numbers = num_category.index.values
    ...
    graphs = [
...

        {
            ...
            'layout': {
                'title': 'Messages by Number of Categories',
                'yaxis': {
                    'title': "Count"
                },
                'xaxis': {
